...
// LEGACY: This script currently saves to Notion. In the future it should be
// updated to save directly to Cloudflare D1 instead.
...
import {
  getDatabaseSchema,
  ensureMissingProperties,
  getExistingMealNames,
  createMealPage,
} from './lib/notion-meals.js'
...
async function main() {
  ...
  if (dryRun) {
    ...
    for (let i = 0; i < meals.length; i++) {
      ...
      console.log(
        `   Kuchnia: ${m.kuchnia} | Trudność: ${m.trudnosc} | Czas: ${m.czas_przygotowania} min`
      )
      console.log(
        `   Kcal baza: ${m.makro?.baza?.kcal} | Kcal z mięsem: ${m.makro?.z_miesem?.kcal}`
      )
      console.log(
        `   Białko baza: ${m.makro?.baza?.bialko}g | Białko z mięsem: ${m.makro?.z_miesem?.bialko}g`
      )
